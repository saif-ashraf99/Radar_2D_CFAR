...
%it a value of 1, else equate it to 0.
   ...
   % CFAR
r_size = 2 * (Tr + Gr) + 1;  % range size
d_size = 2 * (Td + Gd) + 1;  % doppler size
signal_CFAR = zeros(Nr/2, Nd);
Tcell = r_size * d_size - (2 * Gr + 1) * (2 * Gd + 1);

for i = 1+Tr+Gr:(Nr/2 - (Gr+Tr))
    for j = 1+Td+Gd:(Nd - (Gd+Td))
        sumWithoutTcell = sum(db2pow(RDM(i-Gr:i+Gr, j-Gd:j+Gd)), 'all');
        sumWithTcell = sum(db2pow(RDM(i-(Tr+Gr):i+Tr+Gr, j-(Td+Gd):j+Td+Gd)), 'all');
        
        noise_level = sumWithTcell - sumWithoutTcell;

        threshold = noise_level / Tcell;
